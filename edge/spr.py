...
script_directory = os.path.join(os.path.dirname(__file__), 'edge')
main_directory = os.path.dirname(__file__)

Bestip_path = os.path.join(script_directory, "Bestip.txt")
endpoints_path = os.path.join(script_directory, "endpoints.csv")
...
subprocess.run(["wget", url, "-O", os.path.join(script_directory, "warp")])
os.chmod(os.path.join(script_directory, "warp"), 0o755)

command = f"{os.path.join(script_directory, 'warp')} >/dev/null 2>&1"
print("Scanning IPs...")
...
Bestip = []
with open(endpoints_path, "r") as csv_file:
    next(csv_file)
    c = 0
...
with open(os.path.join(main_directory, "warp.json"), "w") as op:
    op.write(
...
def export_SingBox(t_ips):
    with open(os.path.join(script_directory, "edge/singbox-template.json"), "r") as f:
        data = json.load(f)
    ...
    with open(os.path.join(main_directory, "sing-box.json"), "w") as f:
...
def main(script_dir):
    try:
        ...
        url = f"https://gitlab.com/Misaka-blog/warp-script/-/raw/main/files/warp-yxip/warp-linux-{arch}"
        subprocess.run(["wget", url, "-O", os.path.join(script_dir, "warp")], check=True)
        os.chmod(os.path.join(script_dir, "warp"), 0o755)
        print("Scanning IPs...")
        subprocess.run(
            [os.path.join(script_dir, "warp")], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        print("Warp executed successfully.")
        ...
        if os.path.exists(os.path.join(script_dir, "warp")):
            os.remove(os.path.join(script_dir, "warp"))
        ...
        with open(os.path.join(main_directory, "endpoints.csv"), "w") as f:
            f.write(open(endpoints_path, "r").read())
        export_SingBox(top_ips)
        
        # حفظ فایل‌ها در پوشه edge
        # اگر نیاز به حذف دارید، کامنت را بردارید:
        # os.remove(endpoints_path)

if __name__ == "__main__":
    main_directory = os.path.dirname(__file__)
    main(os.path.join(main_directory, 'edge'))
